...
class GooseProvider extends Provider {
  static label = 'Goose CLI';

  constructor(config) {
    super(config);
    this.command = config.command || 'goose';
    this.args = config.args || ['run'];
  ...
  async chatStream(messages, model, onToken, onDone, onError, { signal } = {}) {
    ...
    return new Promise((resolve, reject) => {
      const child = spawn(this.command, [...this.args, '-p', prompt], {
